...
def run_lightgbm(self, X_train, Y_train, X_test) -> float:
  """
  Returns a prediction using lightGBM
  """
  model = lgb.LGBMRegressor(objective = "regression", n_estimators = 500, num_leaves = 300, metric = "rmse", learning_rate = 0.4)
  model.fit(X_train, Y_train)
  Y_hat = model.predict(X_test)[0]
  return Y_hat

def run_knn(self, X_train, Y_train, X_test) -> float:
  """
  Returns a prediction using KNN
  """
  model = KNeighborsRegressor(n_neighbors = 3, weights = "distance", metric = "manhattan")
  model.fit(X_train, Y_train)
  Y_hat = model.predict(X_test)[0]
  return Y_hat
...
class Backtest:
  """
  Runs backtesting on the provided data for a supplied model and provides helper methods.
  """
  def __init__(self):
  ...
  #def __call__(self, output_predication_csv = False, **myModel):
    """
    ...
    for items in self.predictions_info:
      ...
      Y_hat = self.myModel(X_train[-7:], Y_train[-7:], X_test)
    ...
    RMSPE = self.RMSPE()
    if output_predication_csv: self.output_csv()
    return RMSPE
